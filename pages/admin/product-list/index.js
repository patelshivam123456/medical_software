...
const ProductList=(props)=> {
  ...
  const { data, mutate } = useSWR(
    `/api/products?page=${page}&limit=${limit}&sort=${sort}&q=${search}`,
    fetcher
  );
  ...
  function downloadPDF(data) {
    ...
    const rows = (data || []).map((p) => [p.name, p.mrp, p.rate, p.discount]);
    ...
    <div className="p-6">
      ...
      <div className="flex gap-3 mb-4">
        ...
        <button
          ...
          className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow hover:bg-blue-700 cursor-pointer disabled:opacity-50"
        ...
        <button
          ...
          className="bg-green-600 text-white px-4 py-2 rounded-lg shadow hover:bg-green-700 cursor-pointer disabled:opacity-50"
          disabled={loadingBtn === "download"}
        >
          {loadingBtn === "download" ? "Downloading..." : "Download PDF"}
        </button>
        <button
          onClick={() => { setLoadingBtn("refresh"); mutate().finally(()=>setLoadingBtn("")); }}
      ...
      {/* Table */}
      <table className="w-full border" id="productTable">
        <thead>
          <tr className="bg-gray-200">
            {["name","company","salt","mrp","rate","discount"].map(col => (
              <th
                ...
                onClick={() => setSort(`${col}:${sort.endsWith("asc")?"desc":"asc"}`)}
              >
                {col.toUpperCase()}
              </th>
            ))}
          ...
          {items.map((p) => (
            <tr key={p._id}>
              ...
              <td className="border px-2 py-1 flex gap-2">
                <button
                  onClick={() => onEdit(p)}
                  className="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-600 cursor-pointer disabled:opacity-50"
                ...
                <button
                  ...
                  className="bg-red-600 text-white px-2 py-1 rounded hover:bg-red-700 cursor-pointer disabled:opacity-50"
                  disabled={loadingBtn === "delete"}
                >
                  {loadingBtn === "delete" && deleteId === p._id ? "Deleting..." : "Delete"}
                </button>
              </td>
            </tr>
          ))}
      ...
      <div className="flex gap-2 mt-4">
        <button
          disabled={page <= 1 || loadingBtn === "page"}
          onClick={() => { setLoadingBtn("page"); setPage(p => p - 1); setLoadingBtn(""); }}
        ...
        {Array.from({ length: totalPages }, (_, i) => (
          <button
            key={i}
            onClick={() => { setLoadingBtn("page"); setPage(i + 1); setLoadingBtn(""); }}
            className={`px-3 py-1 border rounded cursor-pointer ${page === i + 1 ? "bg-blue-500 text-white" : ""}`}
            disabled={loadingBtn === "page"}
          >
            {i + 1}
          </button>
        ))}
        <button
          disabled={page >= totalPages || loadingBtn === "page"}
          onClick={() => { setLoadingBtn("page"); setPage(p => p + 1); setLoadingBtn(""); }}
          className="px-3 py-1 border rounded disabled:opacity-50 cursor-pointer"
        >
          Next
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <form onSubmit={onSubmit} className="flex flex-col gap-2">
              {["name","company","salt","mrp","rate","discount"].map(f => (
                <input
                  key={f}
