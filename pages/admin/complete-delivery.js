...
const OnlineOrder = (props) => {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const url = tab === 'pending' ? '/api/order/readydelivery' : '/api/order/completeorder';
  ...
  const handleApprove = async () => {
    try {
      const res = await axios.put('/api/order/completeorder', { orderId });
      if (res.status === 200) {
  ...
  return (
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order._id}>
                  ...
                  <td className="border px-3 py-2 text-center">
                    {order.statusUpdatedAt?order.statusUpdatedAt.split("T")[0]: '-'}
                  </td>
                  {/* Action */}
                  {tab === 'pending' && (
                    <td className="border px-3 py-2 text-center">
    ...
    <div>
        <ConfirmationModal loading={loading} title={"Are you sure want to deliverd this Order?"}
