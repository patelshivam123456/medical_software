...
const  ManageStockPage=()=> {
  ...
  //       t.company?.toLowerCase().includes(searchText) ||
  //       t.salt?.toLowerCase().includes(searchText) ||
  //       t.expiry?.toLowerCase().includes(searchText) ||
  //       t.category?.toLowerCase().includes(searchText) ||
  //       t.mg?.toString().toLowerCase().includes(searchText)
  //     );
  //   });
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        saltDropdownRef.current && !saltDropdownRef.current.contains(event.target)
      ) {
  ...
  //   const csv = [
  //     ['Name', 'Quantity', 'Price'],
  //     ...filteredTablets.map(t => [t.name, t.quantity, t.price])
  ...
  const paginatedTablets = filteredTablets.slice(startIndex, startIndex + tabletsPerPage);
      ...
      <form onSubmit={handleSubmit} className="mb-6 w-full max-w-7xl gap-5">
        <div className='flex flex-wrap -mx-3 mb-6'>
          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Tablet Name" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>

          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            <div className='pb-1 text-base'>Packaging</div>
            <input type="text" value={packaging} onChange={(e) => setPacking(e.target.value)} placeholder="Packing" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>

          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0 relative" ref={categoryDropdownRef}>
          ...
          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0 relative" ref={companyDropdownRef}>
            <div className='flex justify-between items-center'>
              <div className='pb-1 text-base'>Company Name</div>
              <div className='text-red-400 text-sm cursor-pointer' onClick={() => setShowCompanyModal(true)}>+Add Company</div>
            </div>
            <input
              type="text"
          ...
          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0 relative" ref={saltDropdownRef}>
            <div className='flex justify-between items-center'>
              <div className='pb-1 text-base'>Salt</div>
              <div className='text-red-400 text-sm cursor-pointer' onClick={() => setShowSaltModal(true)}>+Add Salt</div>
            </div>
            <input
              type="text"
          ...
          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            ...
            <input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} placeholder="Quantity" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>

          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            <div className='pb-1 text-base'>MRP</div>
            <input type="number" value={mrp} onChange={(e) => setMrp(e.target.value)} placeholder="MRP" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>

          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            <div className='pb-1 text-base'>Rate</div>
            <input type="number" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="Rate" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>
          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            <div className='pb-1 text-base'>Mg</div>
            <input type="number" value={mg} onChange={(e) => setMg(e.target.value)} placeholder="mg" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>
          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            <div className='pb-1 text-base'>Batch Number</div>
            <input type="text" value={batch} onChange={(e) => setBatch(e.target.value)} placeholder="Rate" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>
          <div className="w-full md:w-[30%] px-3 mb-6 md:mb-0">
            ...
            <input type="text"
                  ...
                  placeholder="MM/YY" className="block w-full text-black bg-gray-200 border border-red-500 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white" required />
          </div>
        </div>
 ...
 <div className='border p-6 mt-10'>
      <div className="mb-4 flex justify-between items-center">
        <input type="text" placeholder="Search by name..." value={search} onChange={(e) => setSearch(e.target.value)} className="border px-3 py-2 rounded w-1/2" />
      ...
      {/* <table className="w-full border mb-4">
        <thead className="bg-gray-100">
          ...
          <th className="border p-2 text-black">Expiry</th>
            ...
            <th className="border p-2 text-black">Comapny</th>
        ...
        <tbody>
          {paginatedTablets.map((tab,index) => (
            <tr key={tab._id}>
              ...
              <td className="border p-2 text-center">{tab.createdAt&&tab?.createdAt?.split("T")[0]}</td>
              <td className="border p-2 text-center">{tab.updatedAt&&tab?.updatedAt?.split("T")[0]}</td>
              <td className="border p-2 text-center">
                <button onClick={() => handleEdit(tab)} className="text-white mr-4 cursor-pointer"><PencilSquareIcon className='h-4 w-4'/></button>
                <button onClick={() => handleDelete(tab._id)} className="text-red-600 cursor-pointer"><TrashIcon className='h-4 w-4'/></button>
              </td>
            </tr>
          ))}
        </tbody>
...
<table className="w-full border mb-4">
      <thead className="bg-gray-100">
        ...
        <tr>
          ...
          <td className="border p-1">
            <input type="text" value={filters.expiry} onChange={(e) => handleFilterChange(e, "expiry")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.name} onChange={(e) => handleFilterChange(e, "name")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.packaging} onChange={(e) => handleFilterChange(e, "packaging")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.category} onChange={(e) => handleFilterChange(e, "category")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.company} onChange={(e) => handleFilterChange(e, "company")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.salt} onChange={(e) => handleFilterChange(e, "salt")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.quantity} onChange={(e) => handleFilterChange(e, "quantity")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.mrp} onChange={(e) => handleFilterChange(e, "mrp")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.price} onChange={(e) => handleFilterChange(e, "price")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.createdAt} onChange={(e) => handleFilterChange(e, "createdAt")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1">
            <input type="text" value={filters.updatedAt} onChange={(e) => handleFilterChange(e, "updatedAt")} className="w-full p-1 border rounded text-black" />
          </td>
          <td className="border p-1"></td>
        </tr>
      </thead>
      <tbody>
        {paginatedTablets.map((tab, index) => (
          <tr key={tab._id}>
            ...
            <td className="border p-2 text-center">₹{tab.mrp}</td>
            <td className="border p-2 text-center">₹{tab.price}</td>
            <td className="border p-2 text-center">{tab.createdAt?.split("T")[0]}</td>
            <td className="border p-2 text-center">{tab.updatedAt?.split("T")[0]}</td>
            <td className="border p-2 text-center">
              <button onClick={() => handleEdit(tab)} className="text-white mr-4 cursor-pointer"><PencilSquareIcon className='h-4 w-4' /></button>
              <button onClick={() => handleDelete(tab._id)} className="text-red-600 cursor-pointer"><TrashIcon className='h-4 w-4' /></button>
            </td>
      ...
      <div className="flex justify-center gap-2">
        {[...Array(totalPages)].map((_, i) => (
          <button key={i} onClick={() => setCurrentPage(i + 1)} className={`px-3 py-1 rounded ${currentPage === i + 1 ? 'bg-blue-600 text-white' : 'bg-gray-200 text-black'}`}>
            {i + 1}
          </button>
        ))}
      </div>
